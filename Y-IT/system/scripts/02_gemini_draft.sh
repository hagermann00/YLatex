...
# --- Configuration ---
TOPIC_DIR=$1
SCRIPT_NAME="02_gemini_draft"
INPUT_FILENAME="01_research_input.md"
OUTPUT_FILENAME="02_gemini_draft.md"
...
if [ "$MODE" == "manual" ]; then
  echo "Manual mode: Please place your draft file at:"
  echo "$MANUSCRIPTS_DIR/$OUTPUT_FILENAME"
  read -p "Press enter to continue once the file is in place..."
elif [ "$MODE" == "auto" ]; then
  echo "Auto mode: Simulating API call to generate Gemini draft..."
  echo "# Simulated Gemini Draft for $(basename $TOPIC_DIR)" > "$MANUSCRIPTS_DIR/$OUTPUT_FILENAME"
  echo "Simulated draft file created."
fi
