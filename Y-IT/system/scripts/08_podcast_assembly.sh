...
# --- Configuration ---
TOPIC_DIR=$1
SCRIPT_NAME="08_podcast_assembly"
OUTPUT_FILENAME="podcast_script.md"
...
if [ "$MODE" == "manual" ]; then
  echo "Manual mode: Please place your final podcast script at:"
  echo "$OUTPUT_DIR/$OUTPUT_FILENAME"
  read -p "Press enter to continue once the file is in place..."
elif [ "$MODE" == "auto" ]; then
  echo "Auto mode: Simulating podcast script generation..."
  touch "$OUTPUT_DIR/$OUTPUT_FILENAME"
  echo "Simulated podcast script file created."
fi
